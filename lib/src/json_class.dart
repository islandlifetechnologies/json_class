...
abstract class JsonClass {
  ...
  static final _logger = Logger('JsonClass');
  ...
  static Map<String, T>? maybeFromDynamicMapWithKey<T>(
    ...
    if (map != null) {
      result = {};
      for (final entry in map.entries) {
        result[entry.key] = builder(entry.value, key: entry.key);
      }
    }
  ...
  static DateTime? maybeParseDateTime(dynamic value) {
    ...
    if (value != null) {
      ...
      for (final pattern in utcPatterns) {
        try {
          result = DateFormat(pattern).parse(value, true);
          break;
        } catch (e) {
          // no-op
        }
  ...
  static double? maybeParseDouble(dynamic value, [double? defaultValue]) {
    double? result;
    try {
      if (value is String) {
  ...
  static dynamic maybeParseJson(dynamic value) {
    ...
    try {
      ...
      _logger.severe('Error parsing value: [$value]', e, stack);
    }
  ...
  static DateTime? maybeParseUtcMillis(dynamic value) {
    ...
    if (input != null) {
      result = DateTime.fromMillisecondsSinceEpoch(input, isUtc: true);
    }
      ...
      for (final entry in input.entries) {
        if (entry.value != null) {
          if (entry.value is Map) {
            final processed = removeNull(entry.value, removeEmptyCollections);
            if (processed != null) {
